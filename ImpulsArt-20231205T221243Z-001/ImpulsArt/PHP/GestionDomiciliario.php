...
<div class="container-fluid">
    ...
    <div class="row">
        ...
        <button type="button" class="bot w-75 btn btn-success"  style="font-size: 35px;" onclick="RedirigirCrearDomiciliario()">Crear nuevo Domiciliario +</button>

        <script>
function RedirigirCrearDomiciliario() {
  window.location.href = "../CrearDomiciliario.html";
}
</script>
        ...
        <div class="col-4 col-12 col-md-4 ml-3">
            ...
            <form action="GestionDomiciliario.php" method="get">
...
if (isset($_GET['buscar'])) {
  ...
  WHERE Nombre LIKE '$buscar%' OR Apellidos LIKE '$buscar%'
   ORDER BY PkCod_domiciliario";
    ...
    while ($mostrar = mysqli_fetch_assoc($query)) {
        ?>
          <tr>
          <td><?php echo $mostrar['PkCod_domiciliario']?></td>
      <td><?php echo $mostrar['Nombre']?></td>
      <td><?php echo $mostrar['Apellidos']?></td>
      <td><?php echo $mostrar['Salario']?></td>
      <td><?php echo $mostrar['EntregasPendientes']?></td>
      <td><?php echo $mostrar['TipoVehiculo']?></td>
      <td>
        <div>
        <?php echo "<a class='btn btn-outline-primary' href='EditarDomiciliario.php?id=" . $mostrar['Pk_Identificacion'] . "'>EDITAR</a>";?>
        <?php echo "<a class='btn btn-outline-danger' href='EliminarDomiciliario.php?id=" . $mostrar['Pk_Identificacion'] . "'>BORRAR</a>";?>
       </div>
    </td>
   </tr>
        <?php
  ...
  ORDER BY PkCod_domiciliario";
    ...
    $select = "SELECT usuario.Pk_Identificacion, usuario.Nombre, usuario.Apellidos, vehiculo.pk_placa, domiciliario.Salario, domiciliario.PkCod_domiciliario, domiciliario.EntregasPendientes, vehiculo.TipoVehiculo FROM usuario
    inner join domiciliario on usuario.Pk_Identificacion = domiciliario.Fk_identificacion
    inner join vehiculo on domiciliario.fk_placa = vehiculo.pk_placa";
    $query = mysqli_query($conectar, $select);
